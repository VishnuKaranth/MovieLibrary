...
export default function HorrorMoviesSection() {
  ...
  const loadMovies = useCallback(async () => {
    ...
    setError('');
    try {
      const fetchedMovies = await MovieService.getMoviesByGenre(HORROR_GENRE_ID);
  ...
  return (
    <section className="relative py-8 bg-gray-950" id="horror">
      <h2 className="text-3xl font-extrabold mb-6 px-6 sm:px-12 text-white tracking-wide drop-shadow-lg">
        <div className="inline-block transform -skew-x-12 bg-emerald-950 px-4 py-1">Horror Movies</div>
      </h2>
