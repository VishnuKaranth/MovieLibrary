...
// OOP: Encapsulate movie fetching
class MovieService {
...
// Component for each movie slide wrapper
...
export default function ActionMoviesSection() {
  ...
  const loadMovies = useCallback(async () => {
    ...
    setError('');
    try {
      const fetchedMovies = await MovieService.getMoviesByGenre(ACTION_GENRE_ID);
  ...
  return (
    <section className="relative py-8 bg-gray-950" id="action">
      <h2 className="text-3xl font-extrabold mb-6 px-6 sm:px-12 text-white tracking-wide drop-shadow-lg">
        <div className="inline-block transform -skew-x-12 bg-green-500 px-4 py-1">Action Movies</div>
      </h2>
